...
(defhydra +mr/lsp-traverse-hydra (:hint nil)
  "Traverse references"
  ("d" lsp-ui-peek-find-definitions "next" :bind nil :color blue)
  ("n" (-let [(i . n) (lsp-ui-find-next-reference)]
         (if (> n 0) (message "%d/%d" i n))) "next" :color blue)
  ("p" (-let [(i . n) (lsp-ui-find-prev-reference)]
         (if (> n 0) (message "%d/%d" i n))) "prev" :color blue)
  ("R" (-let [(i . n) (lsp-ui-find-prev-reference
                       (lambda (x)
                         (/= (logand (gethash "role" x 0) 8) 0)))]
         (if (> n 0) (message "read %d/%d" (+ i 1) n))) "prev read"
         :bind nil :color green)
  ("r" (-let [(i . n) (lsp-ui-find-next-reference
                       (lambda (x)
                         (/= (logand (gethash "role" x 0) 8) 0)))]
         (if (> n 0) (message "read %d/%d" (+ i 1) n))) "next read"
         :bind nil :color green)
  ("W" (-let [(i . n) (lsp-ui-find-prev-reference
                       (lambda (x)
                         (/= (logand (gethash "role" x 0) 16) 0)))]
         (if (> n 0) (message "write %d/%d" (+ i 1) n))) "prev write"
         :bind nil :color red)
  ("w" (-let [(i . n) (lsp-ui-find-next-reference
                       (lambda (x)
                         (/= (logand (gethash "role" x 0) 16) 0)))]
         (if (> n 0) (message "write %d/%d" (+ i 1) n))) "next write"
         :bind nil :color red)
  ("q" nil "stop" :color white)
  )
